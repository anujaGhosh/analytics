...
frequentItems = eclat (Groceries, parameter = list(supp = 0.01, minlen= 2, maxlen = 2)) #s=0.01, 100 transactions should have the items burger and frensh fries for B->FF
...
rules
inspect (rules[1:5])#{baking powder}=>{wholemilk} support=0.009 conf=0.522 count=91....54%of people buying baking powed got wholemilk
#Sort Rules by confidence, lift and see the data
rulesc <- sort (rules, by="confidence", decreasing=TRUE)
inspect(rulesc[1:5])
rulesl <- sort (rules, by="lift", decreasing=TRUE)#lift=n(A&B)/n(A)*n(B)...should be more than 1...strong association, sale of one affect sale of others
inspect (rulesl[1:5])
...
subset1 = subset(rules2, appearance = list (default="rhs",lhs="whole milk"))
inspect(subset1)

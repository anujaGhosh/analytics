...
head(data)
tail(data)
table(data$Married)#find the number of married and unmarried people
...
colnames(data)
class(data$Age)
apply(data, 2, FUN= class)  #are all numeric...kind of for loop
dim(data)
head(data)
summary(data)
...
k1 <- amap::Kmeans(data[,-1],centers=3, iter.max = 200,nstart = 1,  method = c("euclidean"))#-1 is used to remove customer id for this clustering
head(data)
